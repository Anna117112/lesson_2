...
//
    ...
    private static boolean isWin(char symbol){
        return chekColomAndLine(symbol) || chekDiagonal(symbol);
    }

    private static boolean chekColomAndLine(char symbol) {
        for (int i = 0; i < SIZE; i++) {
            int columnWin = 0;
            int lineWin = 0;
            for (int j = 0; j < SIZE; j++) {
//   В этой строке описано если по строке [i][j] лежит наш символ символ
// то увеличиваем нашу переменную на 1, если символ например X  идут не подрят то :0 сброс
                lineWin = (map[i][j] == symbol) ? lineWin + 1 : 0;
                columnWin = (map[i][j] == symbol) ? columnWin + 1 : 0;
                if (lineWin == DOTS_TO_WIN|| columnWin == DOTS_TO_WIN) {
                    return true;
                }
        ...
        private static boolean chekDiagonal(char symbol) {
            int diagonalRight = 0;
            int diagonalLeft = 0;
        for (int i = 0; i < SIZE; i++) {
                diagonalRight =(map[i][i]==symbol) ? diagonalRight+1:0;
                diagonalLeft = (map [i][map.length-1-i]==symbol) ? diagonalLeft + 1 :0;

                        if (diagonalLeft == DOTS_TO_WIN || diagonalRight == DOTS_TO_WIN){
                return true;
            }
                    }
